...
Euclidean_Distance_POI = POI_Euclidean
with arcpy.EnvManager(extent= extracted_DEM, snapRaster=extracted_DEM):
    POI_Euclidean = arcpy.sa.EucDistance(in_source_data= points_of_interest, maximum_distance=None, cell_size= extracted_DEM, out_direction_raster= "", distance_method="GEODESIC", in_barrier_data="", out_back_direction_raster= "")
    POI_Euclidean.save(Euclidean_Distance_POI)
arcpy.AddMessage("Step 5: POI EUC Complete. (5/9)")

# Step 6: Does Con analysis for the facilities point layer
facility_con_layer = arcpy.sa.Con(in_conditional_raster= Euclidean_Distance_POI, in_true_raster_or_constant= 1, in_false_raster_or_constant= 0, where_clause="VALUE <= {0}".format(str(distance_from_facilities)))
...
Euclidean_Distance_Road = Road_Euclidean
with arcpy.EnvManager(extent= extracted_DEM, snapRaster= extracted_DEM):
    Road_Euclidean = arcpy.sa.EucDistance(in_source_data= road_network, maximum_distance=None, cell_size= extracted_DEM, out_direction_raster= "", distance_method="GEODESIC", in_barrier_data="", out_back_direction_raster= "")
    Road_Euclidean.save(Euclidean_Distance_Road)
arcpy.AddMessage("Step 7: Road EUC Complete. (7/9)")

# Step 8: Does Con analysis for the road layer
road_con_layer = arcpy.sa.Con(in_conditional_raster= Euclidean_Distance_Road, in_true_raster_or_constant= 1, in_false_raster_or_constant= 0, where_clause="VALUE <= {0}".format(str(distance_from_roads)))
...
removal_list = [erased_area, extracted_DEM, slope_Raster, slope_con_raster, Euclidean_Distance_POI, facility_con_layer, Euclidean_Distance_Road, road_con_layer]
